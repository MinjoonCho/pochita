...
export default function StatsPage() {
  ...
  const catBreakdown = CATEGORIES.map((category) => ({
    ...category,
    sec: categorySeconds[category.id] ?? 0,
  }))
    .filter((category) => category.sec > 0)
  ...
  const achievements = [
    { label: "딴짓 입문자", desc: "첫 세션", emoji: "🏅", done: totalSec > 0 },
    { label: "1시간 딴짓", desc: "한 번에 1시간 이상", emoji: "⏰", done: maxSessionSec >= 3600 },
    { label: "꾸준히 딴짓중", desc: "7일 연속 딴짓", emoji: "🔥", done: days.filter((day) => day.sec > 0).length >= 7 },
  ...
  return (
    <div className="min-h-screen bg-[var(--pochita-bg)] text-[var(--pochita-text)] font-sans pb-24">
      <div className="page-shell pt-6 pb-3">
        <h1 className="text-[26px] font-semibold text-[var(--pochita-text)] leading-snug tracking-tight">
          나의 통계
        </h1>
        <p className="text-[13px] text-gray-500 mt-2">{user.nickname}의 딴짓 기록</p>
      </div>

      <div className="page-shell page-stack">
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: "오늘 딴짓", value: todaySec > 0 ? formatTimeKorean(todaySec) : "0초", sub: "오늘 누적", color: "var(--pochita-orange)" },
            ...
            { label: "최장 기록", value: maxSessionSec > 0 ? formatTimeKorean(maxSessionSec) : "-", sub: "단일 세션", color: "var(--pochita-orange)" },
          ].map((item) => (
            <div key={item.label} className="px-4 py-4 rounded-[28px] bg-white border border-gray-200 shadow-sm flex flex-col justify-between min-h-[112px]">
              ...
              <div>
                <p className="text-[18px] font-semibold mt-2" style={{ color: item.color, letterSpacing: "-0.02em" }}>
                  {item.value}
                </p>
            ...
            {days.map((day) => (
              <div key={day.label} className="flex-1 flex flex-col items-center gap-2">
                <div
                  ...
                  style={{
                    height: `${(day.sec / maxDaySec) * 90}px`,
                    background: day.sec > 0 ? "var(--pochita-orange)" : "#F2F4F6",
                    minHeight: "8px",
                  }}
                />
                  ...
                  <div className="flex-1">
                    ...
                    <div className="h-2 rounded-full overflow-hidden bg-gray-100">
                      <div
                        ...
                        style={{ width: `${(category.sec / maxCatSec) * 100}%`, background: category.color }}
                      />
                    </div>
                  </div>
                </div>
          ...
          <div>
            {achievements.map((achievement, index) => (
              <div
                ...
                className={`flex items-center gap-3 px-4 py-3 ${index !== achievements.length - 1 ? "border-b border-gray-100" : ""}`}
                style={{ opacity: achievement.done ? 1 : 0.6 }}
              >
                ...
                {achievement.done && (
                  <span className="text-[11px] px-2.5 py-1 rounded-lg font-semibold bg-orange-50 text-[var(--pochita-orange)] border border-orange-100">
                    달성
                  </span>
                )}
              </div>
