      ...
      <div className="pt-8 pb-6 page-shell text-center bg-white border-b border-[var(--pochita-border)] shadow-sm">
        <div className="relative flex items-center justify-center mb-0">
          ...
          <button
            ...
            className="absolute right-0 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full border border-[var(--pochita-border)] flex items-center justify-center text-xl bg-orange-50 shadow-sm transition-transform active:scale-95"
          >
            {user.avatarEmoji}
          </button>
        </div>
        ...
        <h1 className="text-5xl font-bold timer-digit mb-3 text-[var(--pochita-text)]">
          {formatTime(todaySeconds)}
        </h1>
        <p className="text-sm font-medium text-[var(--pochita-orange)]">오늘 하루 딴짓한 총 시간</p>
      </div>

      <div className="page-shell mt-4 page-stack">
        <h2 className="text-lg font-medium text-[var(--pochita-text)] px-1 leading-tight">
          어떻게 시험을 망쳐볼까요?
        </h2>
        ...
        <div className="grid grid-cols-1 gap-2 slide-up">
          {sortedCategories.map((category) => (
            <button
              key={category.id}
              onClick={() => handleStartDistraction(category.id)}
              className="group w-full flex items-center justify-between px-3 py-3 bg-white rounded-[28px] border border-[var(--pochita-border)] shadow-sm active:scale-98 transition-all hover:border-[var(--pochita-orange)]"
            >
              <div className="flex items-center gap-3">
                <div
                  className="w-16 h-16 rounded-[24px] flex items-center justify-center text-3xl transition-transform group-hover:scale-110 shrink-0"
